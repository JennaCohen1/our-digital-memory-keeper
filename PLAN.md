...
- This supports the example: upload your daughter's camp photos once, then:
...
- Simple landing with options (all enabled from the beginning):
  ...
  - (Later) Continue with Apple when ready to add it.
...
- Primary actions:
  - "Create new Memory Book" → prompts for name only (no invites here).
...
- Map creation time to `captured_at` / event date, and store imported images as assets with `source = "google_photos"`.

---

This plan keeps the core UX, introduces Memory Books as modular spaces, supports non-Google users from day one, and leaves clean hooks for future invites, Drive export, and Google Photos import.
